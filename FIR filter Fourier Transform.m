...
% we will be using hamming function because of
% our ripple and attenuation values fit for hamming from table


fdelta = abs(f_stop - f_pass)/fs; 
N = 3.3/fdelta; % N formula for hamming 3.3/fdelta from the table

% to check N odd or not I go this way
...
x = noisy_speech;
N_speech = length(x);
t = (0:N_speech-1)/fs;  % time vector for time doamin
...
figure(9);
plot(f(1:Nfft/2), abs(X_filtered(1:Nfft/2)));
